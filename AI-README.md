...
- **`snes/Makefile`** ist die einzige legitime Art, das ROM zu bauen. Er erzeugt automatisch ein `tools/.venv` für `gen_font.py` / `gen_font2.py` / `gen_keymap.py` (Pillow + PyYAML). Die generierten Dateien in `snes/assets/` sind **nicht eingecheckt** — alle per `.gitignore` ausgeschlossen.
- **Generierte Dateien haben Header** (`; Auto-generated by tools/... — do not edit manually.`). Nie direkt patchen.
- **`gen_font.py` erzeugt drei Artefakte** (DejaVu Sans Mono, 8×16-Zellen):
...
- **`gen_font2.py` erzeugt zwei Artefakte** (JetBrains Mono Regular, `tools/fonts/JetBrainsMono-Regular.ttf`, 16×16-Zellen, 2bpp anti-aliased):
  - `font2.inc` — **derzeit ungenutzt**, aber mit sehr weichen, bodenlinien-ausgerichteten Glyphen für künftige Hi-Res-/UI-Experimente. Pro Zeichen 4 × 8×8 Tiles in Reihenfolge TL, TR, BL, BR (also Tile-Indices `C*4 .. C*4+3` für `C = ord(ch) - 0x20`). 2bpp-Shading 0..3 kodiert vier Graustufen aus dem Freetype-Renderer.
  - `font2_preview.png` — Kontaktbogen aller 95 Glyphen mit 8-SNES-Pixel-Raster (1 Tile pro Raster-Zelle). Zeigt die AA-Graustufen und die gemeinsame Grundlinie.
