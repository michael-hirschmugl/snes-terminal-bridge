...
def subtile_to_2bpp(rows_8: list[list[int]]) -> bytes:
    """
    Convert 8 rows of an 8-pixel-wide scanline to a 16-byte SNES 2bpp tile.
    Bitplane 0 carries pixel data; bitplane 1 is all zero.
    Used for BG2 in Mode 5 (fills odd screen pixel columns).
    """
    data = bytearray(16)
    for row, scanline in enumerate(rows_8):
        bp0 = 0
...
def main() -> None:
    ...
    for C, char in enumerate(chars):
        ...
        tile_data_2[C * 2]   = subtile_to_2bpp(top_rows)
        tile_data_2[C * 2 + 1] = subtile_to_2bpp(bot_rows)
    ...
    # Write font2.inc (2bpp, for BG2 — odd screen pixel columns)
    with open(font2_path_out, "w", encoding="utf-8") as f:
        f.write("; Auto-generated by tools/gen_font.py — do not edit manually.\n")
        f.write("; SNES 2bpp, 8×16 characters (2 × 8×8 tiles per character).\n")
        f.write("; Used by BG2 in Mode 5 (odd screen pixel columns).\n")
        f.write(f"; {total_tiles} tiles total, 16 bytes each.\n\n")
        for idx, tile in enumerate(tile_data_2):
            pairs = [f"${tile[j*2]:02X},${tile[j*2+1]:02X}" for j in range(8)]
            f.write(f"    .byte {', '.join(pairs)}  ; tile {idx}\n")
    print(f"font2.inc: {total_tiles} tiles ({total_tiles * 16} bytes)")
