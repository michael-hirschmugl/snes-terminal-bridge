...
├── src/
│   └── main.asm          # SNES Mode 5 ROM — BG1 4bpp wallpaper + BG2 2bpp chars, 30×26 scroll grid (16px margin), interlaced 512×448
...
│   └── linux_wallpaper_512x448_right_4bpp.png  # BG1 wallpaper source image
└── tools/
    ...
    ├── gen_assets.py      # PNG → 4bpp palette/tiles/tilemap binaries (mode5_image pipeline)
...
|---|---|---|---|
| `build/terminal.sfc` | SNES Mode 5 + interlace, BG1 4bpp wallpaper + BG2 2bpp text, 16×16 tiles | 30×26 visible (16px margin all sides), 32-row scroll buffer | 16×16 px (PPU assembles from four 8×8 tiles) |

Effective resolution is 512×448 (interlaced). The wallpaper image fills BG1 (4bpp); anti-aliased text from JetBrains Mono renders on BG2 (2bpp, priority 1) above it. See [`docs/AI-MODE-5-README.md`](docs/AI-MODE-5-README.md) for the Mode 5 graphics details.
...
- **BG1 (wallpaper):** Full-screen 4bpp image from `linux_wallpaper_512x448_right_4bpp.png`, generated by `gen_assets.py`. 16 colours at CGRAM `$00–$0F` (sub-palette 0). Priority 0 — renders behind text.
- **BG2 (text):** 4-colour 2bpp anti-aliased greyscale (transparent, dark grey, light grey, white) at CGRAM `$1C–$1F` (sub-palette 7). Priority 1 — renders in front of the wallpaper.
...
- **VRAM layout (word addresses):**
  ...
  - `$2000–$4FFF` — BG1 wallpaper tiles, 24576 bytes (bank 1), `BG12NBA = $02`
...
- **Wallpaper generation:** `tools/gen_assets.py mode5_image` quantises the PNG to 16 colours, slices into 16×16 super-tiles, flip-deduplicates, dense-packs into `tiles.4bpp.chr`, and writes `tilemap.bin` + `palette.bin`.
- **Data transfer:** DMA on reset — BG1 palette (32 bytes) to CGRAM `$00`, text palette (8 bytes) to CGRAM `$1C`, font tiles (6144 bytes) to VRAM `$0000`, BG1 tiles (24576 bytes, bank 1) to VRAM `$2000`, BG1 tilemap (2048 bytes, bank 1) to VRAM `$5000`.
...
The output ROM is a 2-bank LoROM SNES image, exactly 65536 bytes. The first `make` also auto-generates the wallpaper assets (`build/mode5_wallpaper_4bpp/`) via `gen_assets.py`.

### ROM header and hardware notes

The generated ROM is a standard **PAL LoROM** image suitable for running from a flash cartridge.

- **Title (21 bytes):** `SNES TERMINAL+WALL   ` (space-padded)
