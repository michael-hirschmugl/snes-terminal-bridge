...
├── snes.cfg              # ld65 linker config (2-bank LoROM, 64 KiB)
├── src/
│   └── main.asm          # SNES Mode 5 ROM — BG1 4bpp retro border frame + BG2 2bpp chars, 30×26 scroll grid (16px margin), interlaced 512×448
├── assets/               # Source + generated files (generated not committed)
│   ├── font.inc          # 2bpp 16×16 dense-packed tile data (gen_font.py, JetBrains Mono)
│   ├── keymap.inc        # Button bitmask → VRAM tile word lookup (gen_keymap.py)
│   ├── font_preview.png  # Contact sheet of all glyphs (debug artefact)
│   └── linux_wallpaper_512x448_right_4bpp.png  # Former BG1 wallpaper source (not in main build)
└── tools/
...
The `snes/` subdirectory contains the SNES-side application that runs inside the emulator. The ROM is a 64 KiB 2-bank LoROM image built with the cc65 toolchain (`ca65` + `ld65`).

| Output | Graphics mode | Grid | Character cell |
|---|---|---|---|
| `build/terminal.sfc` | SNES Mode 5 + interlace, BG1 4bpp retro border frame + BG2 2bpp text, 16×16 tiles | 30×26 visible (16px margin all sides), 32-row scroll buffer | 16×16 px (PPU assembles from four 8×8 tiles) |

Effective resolution is 512×448 (interlaced). BG1 shows a retro SNES RPG-style border frame (blue gradient + gold ◆ corner ornaments, `gen_border.py`); anti-aliased text from JetBrains Mono renders on BG2 (2bpp, priority 1) above it. See [`docs/AI-MODE-5-README.md`](docs/AI-MODE-5-README.md) for the Mode 5 graphics details.
...
- **BG mode:** SNES Mode 5 + interlace, BG1 (4bpp) + BG2 (2bpp), both 16×16 tiles; both layers on main and sub screen (`TM = TS = $03`) as required by hi-res
- **Resolution:** 512×448 effective (interlaced), enabled via `SETINI = $01`
- **BG1 (border frame):** SNES RPG-style retro frame, generated by `gen_border.py`. Blue gradient (dark navy outer → icy blue inner highlight) with gold ◆ diamond ornaments at all 4 corners. 4 unique 16×16 super-tiles (corner, h-edge, v-edge, blank); the other 3 corners are H/V flips of the corner tile. 9 colours at CGRAM `$00–$0F` (sub-palette 0, indices 9–15 unused). Priority 0 — renders behind text. 1024 bytes of tile data (32 slots × 32 bytes).
- **BG2 (text):** 4-colour 2bpp anti-aliased greyscale (transparent, dark grey, light grey, white) at CGRAM `$1C–$1F` (sub-palette 7). Priority 1 — renders in front of the border.
...
- **Grid:** 30 writable columns × 26 visible rows (columns 1–30, 16px margin on all sides); 32-row circular buffer with BG2VOFS scroll
- **VRAM layout (word addresses):**
  - `$0000–$0BFF` — BG2 font tiles, 6144 bytes, dense-packed (384 × 8×8 slots)
  - `$1000–$13FF` — BG2 tilemap (32×32 entries × 2 bytes = 2 KB), `BG2SC = $10`
  - `$2000–$20FF` — BG1 border tiles, 1024 bytes (bank 1, 32 slots), `BG12NBA = $02`
  - `$5000–$53FF` — BG1 tilemap, 2048 bytes (bank 1), `BG1SC = $50`
- **Tile numbering (BG2 text):** for ASCII char `c`: `C = ord(c) - 0x20`, `tile_word = ((C // 8) * 32 + (C % 8) * 2) | $3C00` (adds priority=1, palette=7). The keymap lookup table (`keymap.inc`) stores this pre-encoded word directly.
- **Font generation:** `tools/gen_font.py` renders JetBrains Mono Regular into 16×16 cells with FreeType anti-aliasing, splits each glyph into four 8×8 sub-tiles, encodes them 2bpp, and places them at VRAM addresses following the `N, N+1, N+16, N+17` pattern. The natural advance width (~10 px at size 16) is auto-detected and the glyph is scaled horizontally via LANCZOS to fill the full 16 px cell width. `RENDER_W` in `gen_font.py` controls the crop width (default `0` = auto-detect; increase to reduce stretch). See [`docs/AI-MODE-5-README.md`](docs/AI-MODE-5-README.md) for the dense-pack rationale.
- **Border generation:** `tools/gen_border.py` programmatically creates the 4 super-tile pixel grids (no source image), encodes them to 4bpp, and dense-packs them into `build/mode5_border_4bpp/tiles.4bpp.chr` + `tilemap.bin` + `palette.bin`.
- **Data transfer:** DMA on reset — BG1 palette (32 bytes) to CGRAM `$00`, text palette (8 bytes) to CGRAM `$1C`, font tiles (6144 bytes) to VRAM `$0000`, BG1 border tiles (1024 bytes, bank 1) to VRAM `$2000`, BG1 tilemap (2048 bytes, bank 1) to VRAM `$5000`.
- **Circular scroll:** `cursor_y` (0–31) tracks the write row; `top_vram_row` tracks the topmost visible row; `BG2VOFS = (top_vram_row × 16) − 16` (9-bit, wraps at 512). The −16 offset shifts the display down 1 row so tilemap row 31 (never written) shows as the blank 16px top margin. On each scroll, the old `top_vram_row` row is also zero-filled so it becomes the new blank top margin row. New rows are zero-filled (tile 0 = space = all-transparent, wallpaper shows through).
...
```bash
cd snes
make            # build ROM → build/terminal.sfc (65536 bytes)
...
The output ROM is a 2-bank LoROM SNES image, exactly 65536 bytes. The first `make` also auto-generates the border assets (`build/mode5_border_4bpp/`) via `gen_border.py`.

### ROM header and hardware notes

The generated ROM is a standard **PAL LoROM** image suitable for running from a flash cartridge.

- **Title (21 bytes):** `SNES TERMINAL+BORDER ` (space-padded)
...
- Verify ROM size is exactly 65536 bytes (`build/terminal.sfc`)
- Use a PAL console profile on your flash cartridge / setup
- **Note:** The 64 KiB ROM has not yet been tested on the Everdrive — only the earlier 32 KiB single-layer version was hardware-confirmed.
