...
├── src/
│   └── main.asm          # SNES Mode 5 ROM — BG1 4bpp wallpaper + BG2 2bpp chars, 32×26 scroll grid, interlaced 512×448
...
|---|---|---|---|
| `build/terminal.sfc` | SNES Mode 5 + interlace, BG1 4bpp wallpaper + BG2 2bpp text, 16×16 tiles | 32×26 visible, 32-row scroll buffer | 16×16 px (PPU assembles from four 8×8 tiles) |
...
- **Circular scroll:** `cursor_y` (0–31) tracks the write row; `top_vram_row` tracks the topmost visible row; `BG2VOFS = top_vram_row × 16` pixels. New rows are cleared with a 32-word zero-fill (tile 0 = space = all-transparent, so the wallpaper shows through).
